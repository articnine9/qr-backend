...
cartRouter.put("/cartitems/:id", async (req, res) => {
  const { id } = req.params;
  const { updatedItems, updatedCombos, elementType, elementId, newStatus } = req.body;

  // If specific element is being updated
  if (elementType && elementId && newStatus) {
    if (elementType !== "item" && elementType !== "combo") {
      return res.status(400).json({ error: "Invalid elementType. It must be 'item' or 'combo'." });
    }
    ...
    try {
      ...
      const updateQuery = elementType === "item" ? {
        $set: {
          "items.$[elem].status": newStatus
        }
      } : {
        $set: {
          "combos.$[elem].status": newStatus
        }
      };

      const arrayFilters = [{ "elem._id": elementId }];
      ...
      if (result.modifiedCount > 0) {
        return res.status(200).json({ message: `${elementType} status updated successfully.` });
      } else {
        return res.status(404).json({ error: "Element not found or status already up to date." });
      }
    } catch (error) {
      console.error("Error updating element status:", error);
      return res.status(500).json({ error: "Error updating element status" });
    }
  }
